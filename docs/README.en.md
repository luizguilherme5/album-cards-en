...
4. **Settings:** connect [Spotify](SPOTIFY.md) or configure [Plex/Caldera](PLAYERS.md). Select a playback device before testing a card.
...
- Grant permissions only to your identified reader; see [reader setup](READER.md). Do not run the entire app as root.
...
Follow [Linux installation and autostart](LINUX.en.md). After setup, `bash deployment/dev.sh enable plex` or `bash deployment/dev.sh enable spotify` starts the app and player at boot. For both on one USB output, follow the audio handoff section.
...
Read [architecture](ARCHITECTURE.md), [album format](ALBUMS.md), [validation and limitations](VALIDATION.md), and [security](../SECURITY.md).
